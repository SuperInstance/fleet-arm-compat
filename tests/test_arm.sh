...
" 2>/dev/null && { PASS=$((PASS+1)); echo "✅"; } || { echo "❌"; }
...
CE
gcc /tmp/_c_test.c -o /tmp/_c_test && /tmp/_c_test && { PASS=$((PASS+1)); echo "✅"; } || { echo "❌"; }
...
RRS
(cd /tmp/_arm_rust && cargo run 2>/dev/null) && { PASS=$((PASS+1)); echo "✅"; } || { echo "❌"; }
...
GGO
(cd /tmp/_arm_go && go mod init armt 2>/dev/null && go run main.go 2>/dev/null) && { PASS=$((PASS+1)); echo "✅"; } || { echo "❌"; }

# JavaScript
TOTAL=$((TOTAL+1)); echo -n "JavaScript... "
node -e "v=[1,0,-1,1,0,-1,1,1];n=[60];v.forEach(x=>n.push(x===1?n[n.length-1]+4:x===-1?n[n.length-1]-4:n[n.length-1]));console.log(n);" 2>/dev/null | grep -q "64,64,60,64,64,60,64,68" && { PASS=$((PASS+1)); echo "✅"; } || { echo "❌"; }
